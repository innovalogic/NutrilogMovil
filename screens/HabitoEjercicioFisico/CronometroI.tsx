...
import { auth, firestore } from '../../firebase'; // Asegúrate de importar Firebase
...
const CronometroI = () => {
  ...
  const [ejercicio, setEjercicio] = useState('Sentadillas');
  ...
  const ejercicios = ['Sentadillas', 'Peso Muerto', 'Zancadas', 'Elevaciones de talones'];
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      onSnapshot(userRef, async (userSnap) => {
        ...
        await guardarProgresoRutina('tren_inferior', ejercicio, tiempoTotalTrabajo, calorias);
        
        navigation.navigate('RutinaInferior', {
          ejercicio,
          repeticiones,
          tiempoTotal: segundos,
          tiempoDescanso: tiempoDescansoConfigurado,
        });
